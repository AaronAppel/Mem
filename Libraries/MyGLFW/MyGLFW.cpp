...
MyGLFW::MyGLFW(const char* windowTitle, vec2 windowSize, vec2 windowPos)
{
	...
	glfwWindowHint(GLFW_RESIZABLE, GL_TRUE); // user can resize window (windowed). Requires GLFW_DECORATED == true
	...
	glfwWindowHint(GLFW_DECORATED, GL_TRUE); // show windows title bar (icon, title, minimize, maximize, close) (windowed)
