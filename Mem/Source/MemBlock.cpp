...
{ // init data block
    ...
    m_BlockSize = sizeof(GBYTE) * m_CellSize * m_CellCount; // size of block in bytes
    // allocate enough RAM for all cells
    m_Data = (GBYTE*)Procure_RAM(m_BlockSize);
    MemBlock::NullData();
}
// Procure_RAM from OS to store cells
void MemBlock::InitNonUniform(int blockSize)
{
    ...
    // init data block
    m_Data = (GBYTE*)Procure_RAM(m_BlockSize); // allocate blockSize of bytes in RAM
    MemBlock::NullData();
}
// Getters + Setters
// Getters
GBYTE* MemBlock::GetFirst()
{
    return m_Data;
}
